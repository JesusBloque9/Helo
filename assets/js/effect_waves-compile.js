...
class Background extends Entity {
  ...
  draw = context => {
    this.drawGradient(context); // this.drawText(context);
  };
} //*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡/
// Canvas
//*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡*/
...
class Canvas_env {
  constructor({
    ...
    this.setCanvasSize();
    this.setupListeners();
    this.render(); // demo pointer

    this.pointer.addPointerModifier((pointer, tick) => {
      const cx = window.innerWidth / 2 * this.dpr;
      const cy = window.innerHeight / 2 * this.dpr; // const dx = window.innerWidth / 3 * this.dpr;
  ...
  render = () => {
    ...
    this.pointer.update(this); // Cleanup "dead" entities
...
class Cursor extends Entity {
  ...
  draw = ({
    ...
    ctx.beginPath();
    ctx.arc(pointer.position.x, pointer.position.y, this.radius, 0, this.pi2, true);
    ctx.closePath();
    ctx.stroke();
  };
} //*‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡‡/
    ...
    ['mousemove', 'touchmove'].forEach((event, touch) => {
      window.addEventListener(event, e => {
        ...
        const {
          x: px,
          y: py
        } = this.position; // disable the demo modifier if it's been added
...
class PolyWave extends Entity {
  ...
  draw = ({
    ...
    ctx.closePath();
    ctx.fillStyle = this.color; //can change to color
...
class Spring extends Point {
  ...
  setAdjacentForces() {
    ...
    this.attractors.forEach((point, i) => {
      ...
      force.x = x1 - x2;
      force.y = y1 - y2; // apply adjacent forces to current spring
  ...
  setSpringForce() {
    ...
    const fy = (this.oy - this.y) * this.elasticity; // sum forces
  ...
  solveVelocity() {
    ...
    const ay = this.fy / this.mass; // velocity, apply damping then ad acceleration

    this.vx = this.damping * this.vx + ax;
    this.vy = this.damping * this.vy + ay; // add velocity to center and top/left

    this.x += this.vx;
    this.y += this.vy; // reset any applied forces
  ...
  draw = ({
    ...
    ctx.fillStyle = 'white';
    ctx.lineWidth = 5;
    ctx.fillRect(x - 2, y - 2, 4, 4); // ctx.beginPath();
...
const colors = ['#FFFFFF', '#FFFFFF', '#FFCA30', '#DE5307', '#FF2A1B', '#6049F8', '#007CB0'];
const center = new Point(window.innerWidth / 2 * DPR, window.innerHeight / 2 * DPR);

const createWaves = amount => Array(amount).fill(null).map((_, i) => {
